...
WIKIDATA_LABEL_LONG = "rdfs:label"
...
class InvertedIndex(object):
  ...
  def __init__(self, input_path, index_path):
    ...
    LOGGER.error("Input path: " + self.input_path)
    LOGGER.error("Index path: " + self.index_path)
  ...
  def process_line(line):
    ...
    if count % 10000 == 0:
      LOGGER.error("[{}] {} strings added.".format(datetime.datetime.now(), count))
    return (sub, predicate, obj, normalized_obj, n_grams)

  def run(self):
    LOGGER.error("[{}] Starting Indexer...".format(datetime.datetime.now()))
    ...
    for sub, predicate, obj, normalized_obj, n_grams in results:
      if predicate not in [WIKIDATA_LABEL_LONG]:
        continue
      ...
      if predicate == WIKIDATA_LABEL_LONG:
    ...
    LOGGER.error("[{}] Indexing finished. {} strings in total. ".format(datetime.datetime.now(), count))

    LOGGER.error("[{}] Start to writing index to database ...".format(datetime.datetime.now()))
    LOGGER.error("[{}] Stage 1: Write full name index. Total {} index.".format(datetime.datetime.now(),
    ...
    for key in self.full_string_inverted_index:
      ...
      if count % 10000 == 0:
        ...
        LOGGER.error("[{}] {} index added.".format(datetime.datetime.now(), count))
    self.full_collection.close()
    del self.full_string_inverted_index
    LOGGER.error("[{}] Close dbs.".format(datetime.datetime.now()))

    LOGGER.error("[{}] Stage 2: Write name index. Total {} index.".format(datetime.datetime.now(),
